# output: mutation estimates, including confidence intervals
# R utils
library(tidyverse)
library(data.table)
library(Biostrings)

# for blood: take the values from the Watson et al., Science paper
# high estimate: 1.3e6
# mid estimate: 1e5
# low estimate: 2.5e4
...
color_df = lapply(tissue_colors, \(x)
                  data.table(category = names(x), color = x)) |>
